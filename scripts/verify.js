...
TOOLS.forEach(tool => {
  ...
  const importMatch = appTsx.match(new RegExp(`const\\s+(\\w+)\\s+=\\s+lazy\\(\\(\\)\\s*=>\\s*import\\('\\./pages/([^']+)'\\)\\);`));
  let compFile = '';
  ...
  // Home grid and footer map over TOOLS so yes, they are in there automatically if they are in tools.ts.

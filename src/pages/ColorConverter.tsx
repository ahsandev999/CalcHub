...
export default function ColorConverter() {
  ...
  const preview = useMemo(() => {
    ...
    return c ? `rgb(${c.r}, ${c.g}, ${c.b})` : hex || '#6366f1';
  }, [hex]);
  ...
  const updateFromRgb = (key: 'r' | 'g' | 'b', valStr: string) => {
    const newRgb = { ...rgb, [key]: valStr };
    setRgb(newRgb);
    const rNum = parseInt(newRgb.r);
    const gNum = parseInt(newRgb.g);
    const bNum = parseInt(newRgb.b);
    if (!isNaN(rNum) && !isNaN(gNum) && !isNaN(bNum)) {
      setHex(toHex(rNum, gNum, bNum));
      const h = rgbToHsl(rNum, gNum, bNum);
      setHsl({ h: String(h.h), s: String(h.s), l: String(h.l) });
    }
  };

  const updateFromHsl = (key: 'h' | 's' | 'l', valStr: string) => {
    const newHsl = { ...hsl, [key]: valStr };
    setHsl(newHsl);
    const hNum = parseInt(newHsl.h);
    const sNum = parseInt(newHsl.s);
    const lNum = parseInt(newHsl.l);
    if (!isNaN(hNum) && !isNaN(sNum) && !isNaN(lNum)) {
      const c = hslToRgb(hNum, sNum, lNum);
      setRgb({ r: String(c.r), g: String(c.g), b: String(c.b) });
      setHex(toHex(c.r, c.g, c.b));
    }
  };

  const handleConvert = (overrideHex?: string) => {
    const valHex = overrideHex !== undefined ? overrideHex : hex;
    const c = hexToRgb(valHex);
    if (c) {
      const h = rgbToHsl(c.r, c.g, c.b);
      addHistory({
        ...
        expression: valHex,
        result: `RGB(${c.r}, ${c.g}, ${c.b}) | HSL(${h.h}, ${h.s}%, ${h.l}%)`,
      });
    }
  };
  ...
  return (
    <PageTransition className="page-medium">
      ...
      <Card padding="lg">
        ...
        <Input label="HEX" value={hex} onChange={(e) => updateFromHex(e.target.value)} placeholder="e.g. #6366f1" />
        <div className="grid-3">
          <Input label="R" type="number" value={String(rgb.r)} onChange={(e) => updateFromRgb('r', e.target.value)} min="0" max="255" placeholder="e.g. 99" />
          <Input label="G" type="number" value={String(rgb.g)} onChange={(e) => updateFromRgb('g', e.target.value)} min="0" max="255" placeholder="e.g. 102" />
          <Input label="B" type="number" value={String(rgb.b)} onChange={(e) => updateFromRgb('b', e.target.value)} min="0" max="255" placeholder="e.g. 241" />
        </div>
        <div className="grid-3">
          <Input label="H" type="number" value={String(hsl.h)} onChange={(e) => updateFromHsl('h', e.target.value)} min="0" max="360" placeholder="e.g. 239" />
          <Input label="S %" type="number" value={String(hsl.s)} onChange={(e) => updateFromHsl('s', e.target.value)} min="0" max="100" placeholder="e.g. 84" />
          <Input label="L %" type="number" value={String(hsl.l)} onChange={(e) => updateFromHsl('l', e.target.value)} min="0" max="100" placeholder="e.g. 67" />
        </div>
